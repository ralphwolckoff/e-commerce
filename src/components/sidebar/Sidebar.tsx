...
// Les imports non résolus (@/) sont conservés tels quels car ils font partie de votre structure de projet
// L'erreur est ignorée ici, car la structure est nécessaire pour l'exécution dans votre environnement Next.js
import { Role } from "@/common/role.enum";
import { useAuth } from "@/context/AuthContext";
import { useAuthStore } from "@/store/authStore";
import { Typography } from "@/ui/design/typography/Typography";
...
import {
  RiUser3Line,
  RiCoupon5Line, // Utilisé pour 'Mes Promotions' (coupons)
  RiShoppingBagLine,
  RiDashboardLine,
} from "react-icons/ri";
import { LogoutIcon2, TagsIcon } from "../icons";
...
export default function Sidebar() {
  const router = useRouter();
  const pathname = usePathname();
  // Utilisation de useAuth() du contexte pour la fonction de déconnexion
  const { logout } = useAuth();
  const { user, profile } = useAuthStore();
  const [activeTab, setActiveTab] = useState("");
  ...
  const navItems: NavItem[] = [
    {
      name: "Acceuil",
      tab: "dashboard",
      href: user?.role === Role.CLIENT ? "/my-account" : "/mon-espace",
      icon: <RiDashboardLine />,
    },
