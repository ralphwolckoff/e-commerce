...
import { Images, Product } from "@/types/products";
...
const { isModalOpen, closeModal } = useModalContext();
  ...
  const [currentImageIndex, setCurrentImageIndex] = useState(0);
    ...
    const [mainImage, setMainImage] = useState<Images | undefined>(undefined);
  ...
  const handleThumbnailClick = (image: Images, index: number) => {
    setMainImage(image);
  ...
  const handleQuantityChange = (amount: number) => {
    setQuantity((prevQty) => {
      ...
      if (newQty > availableStock) {
        toast.error(
          `Stock insuffisant. Il ne reste que ${availableStock} articles.`
        );
        return prevQty;
      }
